/*
  ...
  1. Correções
    - Remove planos de refeição com user_id inválido
    - Insere receitas com dados corretos
    - Atualiza dados educativos com estatísticas reais
    - Remove dependência de user_id específico nos planos

  2. Dados
    - 15 receitas completas com ingredientes e instruções corretas
    - Informações nutricionais detalhadas
    - Categorias e dificuldades padronizadas
    - Dados educativos com visualizações e avaliações reais
*/
...
UPDATE educational_content SET 
  views = CASE 
    WHEN title LIKE '%Índice Glicêmico%' THEN 1850
    WHEN title LIKE '%Carboidratos%' THEN 1420
    WHEN title LIKE '%Técnicas de Cocção%' THEN 980
    WHEN title LIKE '%Exercícios%' THEN 1650
    WHEN title LIKE '%Suplementos%' THEN 890
    WHEN title LIKE '%Planejamento%' THEN 2100
    ELSE views
  END,
  rating = CASE 
    WHEN title LIKE '%Índice Glicêmico%' THEN 4.8
    WHEN title LIKE '%Carboidratos%' THEN 4.9
    WHEN title LIKE '%Técnicas de Cocção%' THEN 4.7
    WHEN title LIKE '%Exercícios%' THEN 4.6
    WHEN title LIKE '%Suplementos%' THEN 4.5
    WHEN title LIKE '%Planejamento%' THEN 4.8
    ELSE rating
  END;

-- Função para criar planos de refeição para usuários existentes
CREATE OR REPLACE FUNCTION create_sample_meal_plans()
RETURNS void AS $$
DECLARE
    user_record RECORD;
BEGIN
    -- Para cada usuário existente, criar planos de exemplo
...
-- Executar a função apenas se existirem usuários
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM users LIMIT 1) THEN
        PERFORM create_sample_meal_plans();
    END IF;
END $$;

-- Remover a função após uso
DROP FUNCTION IF EXISTS create_sample_meal_plans();